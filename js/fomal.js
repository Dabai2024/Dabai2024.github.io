...
function showWelcome() {
  ...
  let dist = getDistance(106.13952332868176,38.493632649862136,ipLoacation.result.location.lng, ipLoacation.result.location.lat); //同心经纬度105.970555, 36.977054
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 55.6 + 'px',
        nekoImg: "https://tuchuang.voooe.cn/images/2023/12/24/503e00001f9d4feb2eb2bbf903007ad5ca2e3677061e94d5b-pixian.png",
        hoverMsg: "记得健身哦~",
...
B = ["生日快乐，永远幸福！", "吃饭香香，身体棒棒！", "不失眠，不焦虑！", "像风一样自由的生活！", "金钱不缺，微笑不断！", "一切顺心，安逸！", "像孩子一样快乐，像星星一样努力发光，像风一样自由", "天天娃哈哈，岁岁乐百事，笑脸美如花，青春惹人夸。生日快乐！","新的一岁，愿你想要的都拥有，得不到的都释怀，永远保持那份让我羡慕的洒脱与光芒✨（蛋糕分我一口，就当礼物了）","晨有清逸，暮有闲悠,梦随心动，心随梦求（礼物已备好，速报坐标）","愿你的笑容永远和第一次见面时一样明亮✨","颜值不掉线，钱包鼓鼓的！烦恼全清零，桃花……（算了，这个随缘吧😏）生日快乐！礼物记得签收！📦"]
if (m == 4 && dd == 16) {//小猫咪生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
...
function setFont(n) {
  ...
  if (n == "default") {
    ...
    document.body.style.fontFamily = "var(--global-font),-apple-system, IBM Plex Mono ,monosapce,'微软雅黑', sans-seriff";
  }
...
if (localStorage.getItem("transNum") == undefined) {
  localStorage.setItem("transNum", 29);
}
...
if (localStorage.getItem("blurRad") == undefined) {
  localStorage.setItem("blurRad", 24);
}
